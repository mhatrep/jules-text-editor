...
*   **View Menu**:
    ...
    *   **QuickText Transformer (Tools Menu)**:
        *   A powerful tool for batch text transformation based on patterns.
        *   **Input Data**: Area to paste or type multi-line text. If text is selected in the current editor tab when the transformer is opened, it will pre-fill this area.
        *   **Delimiter**: Specify the character (e.g., `,`, `\t`, `|`) or string that separates fields in your input data. Leave blank for line-by-line processing (each line is one field).
        *   **Pattern**: Define the transformation rule using placeholders:
            *   `$1`, `$2`, ... `$N`: Correspond to the Nth field from the input line (split by the delimiter).
            *   `$0`: Represents the entire original input line.
            *   `$lineNumber`: The 1-indexed number of the current input line being processed.
            *   `$tab`: Inserts a tab character.
            *   `$newline`: Inserts a newline character.
            *   `\$`: Use `\$` to insert a literal dollar sign (e.g., `Price: \$$1`).
        *   **Output**: Shows the transformed text.
        *   **Transform Button**: Manually triggers the transformation.
        *   **Copy Output Button**: Copies the entire content of the output area to the clipboard.
        *   **Live Preview Checkbox**: If checked, the output automatically updates (after a short delay) as you modify the input data, pattern, or delimiter.
        *   **Pattern Management**:
            *   **Save Current Pattern**: Allows you to save the current pattern text with a custom name for the current session.
            *   **Load Saved Pattern...**: Opens a dialog to select and load a previously saved pattern from the current session.
            *   Comes with a few pre-defined patterns like "SQL INSERT" and "HTML List".
        *   *Example*:
            *   Input Data: `John,Doe,30\nJane,Roe,25`
            *   Delimiter: `,`
            *   Pattern: `User: $1 $2 is $3 years old. (Line: $lineNumber)`
            *   Output:
                ```
                User: John Doe is 30 years old. (Line: 1)
                User: Jane Roe is 25 years old. (Line: 2)
                ```
