...
class PptxToImageDialog(tk.Toplevel):
    def __init__(self, master):
        ...
        self.image_format_combo = ttk.Combobox(options_frame, textvariable=self.image_format_var, values=["PNG", "JPG", "GIF", "BMP", "TIF"], state="readonly")
        self.image_format_combo.pack(side=tk.LEFT, padx=5)

        ttk.Label(options_frame, text="DPI:").pack(side=tk.LEFT, padx=(10, 5))
        self.dpi_var = tk.StringVar(value="300")
        self.dpi_spinbox = ttk.Spinbox(options_frame, from_=72, to=600, increment=1, textvariable=self.dpi_var, width=8)
        self.dpi_spinbox.pack(side=tk.LEFT, padx=5)
    ...
    def _conversion_worker(self, pptx_file, output_dir, image_format, dpi):
        ...
        try:
            ...
            for i, slide in enumerate(presentation.Slides):
                ...
                progress = (i + 1) / len(presentation.Slides) * 100
            ...
            if isinstance(e, comtypes.COMError):
                self.status_label_var.set("Status: Error - A COM error occurred. Is PowerPoint installed and running correctly?")
            else:
