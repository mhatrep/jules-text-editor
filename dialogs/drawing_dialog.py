...
class DrawingDialog:
    def __init__(self, parent_editor):
        ...
        self.stipple_patterns = ["No Fill", "Solid Fill", "gray75", "gray50", "gray25", "gray12", "hourglass", "info", "questhead", "error", "warning"]
        ...
        self.pen_button.grid(row=2, column=0, padx=1, pady=1, sticky="ew")
        ...
        flowchart_tools_subframe = ttk.Frame(tools_group)
        flowchart_tools_subframe.pack(fill=tk.X, pady=(5,0))
        self.oval_button = ttk.Button(flowchart_tools_subframe, text="O", command=self.activate_oval_mode, width=5)
        self.oval_button.pack(side=tk.LEFT, padx=1)
        self.diamond_button = ttk.Button(flowchart_tools_subframe, text="◇", command=self.activate_diamond_mode, width=5)
        self.diamond_button.pack(side=tk.LEFT, padx=1)
        self.parallelogram_button = ttk.Button(flowchart_tools_subframe, text="▱", command=self.activate_parallelogram_mode, width=5)
        self.parallelogram_button.pack(side=tk.LEFT, padx=1)
    ...
    def draw(self, event):
        ...
        elif self.drawing_mode in ["rectangle", "circle", "triangle", "oval", "diamond", "parallelogram"]:
            if self.shape_start_x is not None and self.shape_start_y is not None:
                ...
                if self.drawing_mode == "rectangle":
                    ...
                    p1 = ((draw_x1 + draw_x2) / 2, draw_y1)
                    p2 = (draw_x2, (draw_y1 + draw_y2) / 2)
                    p3 = ((draw_x1 + draw_x2) / 2, draw_y2)
                    p4 = (draw_x1, (draw_y1 + draw_y2) / 2)
                    self.temp_shape_id = self.canvas.create_polygon(
                        p1, p2, p3, p4,
                        outline=self.current_color, width=self.current_brush_size,
                        fill="", stipple=""
                    )
                elif self.drawing_mode == "parallelogram":
                    offset = (draw_x2 - draw_x1) * 0.2
                    p1 = (draw_x1 + offset, draw_y1)
                    p2 = (draw_x2, draw_y1)
                    p3 = (draw_x2 - offset, draw_y2)
                    p4 = (draw_x1, draw_y2)
                    self.temp_shape_id = self.canvas.create_polygon(
                        p1, p2, p3, p4,
                        outline=self.current_color, width=self.current_brush_size,
    ...
    def stop_draw(self, event):
        if self.drawing_mode == "pen":
            ...
            if self.shape_start_x is not None and self.shape_start_y is not None:
                ...
                selected_pattern = self.current_fill_pattern_var.get()
                fill_color = self.current_color if selected_pattern != "No Fill" else ""
                stipple_option = ""
                if selected_pattern != "Solid Fill" and selected_pattern != "No Fill":
                    stipple_option = selected_pattern

                shape_id = None
                if self.drawing_mode == "rectangle":
                    shape_id = self.canvas.create_rectangle(draw_x1, draw_y1, draw_x2, draw_y2,
                                                            fill=fill_color, outline=self.current_color,
                                                            width=self.current_brush_size, stipple=stipple_option)
                elif self.drawing_mode == "circle":
                    ...
                    p3 = ((draw_x1 + draw_x2) / 2, draw_y2)
                    p4 = (draw_x1, (draw_y1 + draw_y2) / 2)
                    shape_id = self.canvas.create_polygon(p1, p2, p3, p4,
                                                          fill=fill_color, outline=self.current_color,
                                                          width=self.current_brush_size, stipple=stipple_option)
                elif self.drawing_mode == "parallelogram":
                    offset = (draw_x2 - draw_x1) * 0.2
                ...
                if shape_id:
                    group_tag = f"item_group_{self.item_counter}"
                    self.canvas.addtag_withtag(group_tag, shape_id)
                    self.last_drawn_group_tag = group_tag
                    self.item_counter += 1
            self.shape_start_x, self.shape_start_y = None, None
    ...
    def activate_oval_mode(self):
        self._deactivate_all_tools_visual()
        self.drawing_mode = "oval"
        if hasattr(self.oval_button, 'state'): self.oval_button.state(['pressed'])
        self.text_options_frame.pack_forget()

    def activate_diamond_mode(self):
        self._deactivate_all_tools_visual()
        self.drawing_mode = "diamond"
        if hasattr(self.diamond_button, 'state'): self.diamond_button.state(['pressed'])
        self.text_options_frame.pack_forget()

    def activate_parallelogram_mode(self):
        self._deactivate_all_tools_visual()
        self.drawing_mode = "parallelogram"
        if hasattr(self.parallelogram_button, 'state'): self.parallelogram_button.state(['pressed'])
        self.text_options_frame.pack_forget()
    ...
    def _apply_sketch_filter(self):
        img_cv = cv2.cvtColor(np.array(self.original_image), cv2.COLOR_RGBA2BGRA)
        grey_img = cv2.cvtColor(img_cv, cv2.COLOR_BGR2GRAY)
        invert = cv2.bitwise_not(grey_img)
        blur = cv2.GaussianBlur(invert, (21, 21), 0)
        inverted_blur = cv2.bitwise_not(blur)
        sketch = cv2.divide(grey_img, inverted_blur, scale=256.0)
        return Image.fromarray(cv2.cvtColor(sketch, cv2.COLOR_GRAY2BGR))
